...
out=sim('Leg_Sim_VariableDamping',simOpts);
...
harea=area(out.tout,[...
    ...
    -out.E_fric out.E_esFlx(:,4)+out.E_esExt(:,4) out.E_ankle(:,2) ... % internal wasted energy (friction)
...
mycol=[[0.75 0.5 1]; % gravitational potential energy
       ...
       [1;0.75;0.5]*[1 0.75 0.5]; % internal wasted energy (dampers)
...
%plot(t_ctrl,filter(B, A, interp1(t,out.qd(:,1),t_ctrl)),'linewidth',2);
plot(t_ctrl,filtfilt(B, A, interp1(t,out.qd(:,1),t_ctrl)),'linewidth',2);
% com
% plot(t,vcm(:,1));hold on
% plot(t_ctrl,filter(B, A, interp1(t,vcm(:,1),t_ctrl)),'linewidth',2);
...
legend('trunk horizontal velocity',...
    ...'q1d filtered',...
    'trunk horizontal velocity filtfiltered',...
    ...'CoMxd','CoMxd filtered','CoMxd filtfiltered',...
    ...'vplot','vplot filtered','vplot filtfiltered',...
    'reference velocity','estimated velocity','Kthrust/100','states')
